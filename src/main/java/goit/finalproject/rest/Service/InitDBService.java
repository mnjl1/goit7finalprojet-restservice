...
//        проверить наличие базьі, сoздать если ее нет и заполнить
        if (dateEventService.findAllEvents()!=null){
            dateEventService.deleteAll();
        }

        if(dateStatusService.findAllSatuses()!=null){
            dateStatusService.deleteAll();
        }
        if(employeeService.getAll()!=null){
            employeeService.deleteAll();
        }
        if(departmentService.findAllDepartment()!=null){
            departmentService.deleteAll();
        }
        if(positionService.findAllPosition()!=null){
            positionService.deleteAll();
        }
        if(eventService.findAllEvent()!=null){
            eventService.deleteAll();
        }
        if(statusWorkService.findAllStatusWork()!=null){
            statusWorkService.deleteAll();
        }
        if(workReportService.findAll()!=null){
